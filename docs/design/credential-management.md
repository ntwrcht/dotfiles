...
Each wrapper is one line of the form `jira() { secret run JIRA_API_TOKEN -- jira "$@"; }`. `bin/secret` is an external process and cannot see zsh functions, so this doesn't recurse. Adding a tool means adding one line.
